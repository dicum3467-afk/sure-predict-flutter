...
def _upsert_fixture(cur, fx: Dict[str, Any], league_id: str, run_type: str) -> None:
    provider_fixture_id = str(fx.get("provider_fixture_id") or "")
    ...
    computed_at = _as_dt_iso(fx.get("computed_at") or fx.get("computedAt"))
    ...
    # varianta rapidă cu ON CONFLICT (dacă ai UNIQUE pe (provider_fixture_id, league_id, run_type))
    try:
        cur.execute(
            """
            INSERT INTO fixtures (
              id, provider_fixture_id, league_id,
              kickoff_at, status, home, away,
              run_type, computed_at,
        ...
        # fallback UPDATE/INSERT dacă nu ai UNIQUE
        cur.execute(
            """
            SELECT id
            FROM fixtures
        ...
        if row and row[0]:
            cur.execute(
                """
                UPDATE fixtures
                SET kickoff_at=%s, status=%s, home=%s, away=%s, computed_at=%s,
                    p_home=%s, p_draw=%s, p_away=%s, p_gg=%s, p_over25=%s, p_under25=%s
                WHERE id=%s
                """,
                (
                    kickoff_at,
...
def _sync_one_league(league_id: str, season: int, run_type: str, days_ahead: Optional[int]):
    # 0) citim provider_league_id numeric din DB pentru league_id intern (UUID)
    conn = None
    try:
        conn = get_conn()
        cur = conn.cursor()
        ...
        if not row or row[0] is None:
            return 0, "league not found or missing provider_league_id"
    ...
    # 1) fetch fixtures de la provider
    try:
