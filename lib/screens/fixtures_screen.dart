...
class _FixturesScreenState extends State<FixturesScreen> {
  ...
  void initState() {
    super.initState();
    _load();
    // asigură-te că favoritele sunt încărcate
          ...
          body: FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return Center(child: Padding(padding: const EdgeInsets.all(16), child: Text('Eroare: ${snapshot.error}')));
              }

              final data = (snapshot.data ?? <Map<String, dynamic>>[]);
              if (data.isEmpty) {
                WidgetsBinding.instance.addPostFrameCallback((_) => _startOrStopLiveTimer(false));
                      ...
                      children: [
                        Container(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                child: Text(_leagueTitle(leagueId), style: Theme.of(context).textTheme.titleMedium),
                              ),
                              if (liveCount > 0)
                                Container(
